<template>
  <div>
    <div class="text-h5" style="margin-bottom:10px">
      <q-icon name="assignment" size="31px"> </q-icon>
      作业
      ...
      <template v-slot:body="props">
        <q-tr :props="props" @click="props.expand = !props.expand">
          ...
          <q-td
            key="deadline"
            :props="props"
            :class="props.row.deadlineColor"
            >{{ $datetime.getTimeleft(props.row.deadline) }}</q-td
          >
          <q-td key="functions" :props="props">
            ...
            <q-btn-group flat v-if="$q.screen.gt.xs">
              ...
              <q-btn
                ...
                icon="delete"
                label="删除"
                @click="del(props.row.id, props.row.name)"
              />
            </q-btn-group>
    ...
    <q-page-sticky position="top-right" :offset="[30, 30]">
      <div>
        <q-btn fab color="primary" icon="add" to="/teachers/assignments/new">
...
export default {
  ...
  methods: {
    ...
    async del(id, name) {
      const result = await dbWorks.teacherWorksId({
        assignmentId: id
      });
      ...
      if (result.length == 0) {
        this.$q
          .dialog({
            ...
            title: "Success",
            name: "确认",
            text: '您确定要删除作业 "' + name + '" 吗?',
            cancel: true
          })
          .onOk(async () => {
            await dbAssignments.deleteAssignment({
              id: id
            });
            this.init();
          });
    ...
    async details(id, loadingDetails) {
